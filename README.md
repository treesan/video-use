...
**第一层 — 音频转录（始终加载）。** 支持**双 ASR 后端**：英文优先使用 ElevenLabs Scribe，中文优先使用火山引擎 BigASR Turbo。`transcribe.py` 自动选择可用后端，也可通过 `--asr` 参数手动指定。每个源文件获得词级时间戳、说话人分离和音频事件（`(笑声)`、`(掌声)`、`(叹气)`）。所有素材打包成一个 ~12KB 的 `takes_packed.md` — LLM 的主阅读视图。
...
对于无人机航拍、旅行 Vlog 等**无对白**素材，音频转录没有内容，上述两层方案无法工作。video-use 采用**视觉优先**的高光检测方案：

1. **镜头分割** — PySceneDetect（AdaptiveDetector）自动识别镜头切换边界，将长视频拆成独立镜头
2. **质量预筛** — OpenCV 逐帧检查模糊度（Laplacian）、曝光、运动幅度和黑帧，过滤掉 30-50% 的低质镜头
3. **VLM 场景理解** — 通过小米 MiMo 或 MiniMax-M3 等视觉大模型，对每个质量通过的镜头做原生视频理解（非单帧截图），输出场景描述
4. **LLM 评分排序** — 按美学质量和主题相关性对高光镜头评分，输出 `highlights.json`

高光结果直接兼容 edl.json 的 `source/start/end` 格式，LLM 可以像使用转录文本一样使用它来规划剪辑方案。配合 BGM 节拍检测和音频混音，形成完整的静音航拍剪辑流水线。
...
```bash
# 检测静音航拍素材中的高光片段
uv run python3 helpers/highlight_detect.py /path/to/videos --theme "旅行Vlog"
